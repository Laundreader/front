...
import { addLaundryToBasket } from "@/entities/laundry/api";
...
export const Route = createFileRoute("/laundry-solution")({
	...
	errorComponent: () => <Navigate to="/label-analysis" replace />,
	beforeLoad: ({ search }) => search,
	loader: ({ context }) => {
		context.queryClient.ensureQueryData(laundryQueryOptions(context.laundryId));
		context.queryClient.ensureQueryData(
			laundrySolutionQueryOptions(context.laundryId),
		);
	},
...
function RouteComponent() {
	...
	const { data: solution } = useSuspenseQuery(
		laundrySolutionQueryOptions(laundryId),
	);

	const mutation = useMutation({
		mutationFn: () => addLaundryToBasket(laundry),
		onMutate: () => {
			overlay.open(
				({ isOpen, close }) => {
					return (
		...
		onSuccess: async () => {
			const laundryWithSolution: Laundry = {
				...laundry,
				solutions: solution.solutions,
			};

			await addLaundryToBasket(laundryWithSolution);
			queryClient.invalidateQueries({
				queryKey: laundryQueryOptions(laundryId).queryKey,
			});
	...
	const currentSolution = solution.solutions.find(
